module("luci.controller.dashboard.index", package.seeall)
...
local function get_wifi_interfaces()
    ...
    if #ifaces == 0 then
        local net_ls = luci.sys.exec("ls /sys/class/net/ 2>/dev/null")
        if net_ls then
            for iface in net_ls:gmatch("%S+") do
...
function action_api()
    ...
    local net_handle = io.popen("ls -1 /sys/class/net/")
    local interfaces = ""
    if net_handle then
        interfaces = net_handle:read("*a") or ""
        net_handle:close()
    end
    ...
    local ping_result = luci.sys.exec("ping -c 1 -W 1 -q 1.1.1.1 2>/dev/null && echo 'Online' || echo 'Offline'")
    result.internet = {
        status = ping_result:match("Online") and "Online" or "Offline"
    }
