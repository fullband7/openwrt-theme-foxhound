...
local ALLOWED_IPS_CACHE = "/tmp/foxhound/allowed_ips"
local ALLOWED_IPS_TTL = 60
local function get_own_ips()
 local now = os.time()
 local stat = nixio.fs.stat(ALLOWED_IPS_CACHE)
 local raw = nil
 if stat and (now - (stat.mtime or 0)) < ALLOWED_IPS_TTL then
  local f = io.open(ALLOWED_IPS_CACHE, "r")
  if f then
   raw = f:read("*a")
   f:close()
  end
 ...
 if not raw then
  raw = luci.sys.exec("ip -o addr show 2>/dev/null") or ""
  luci.sys.exec("mkdir -p /tmp/foxhound 2>/dev/null")
  local tmp = ALLOWED_IPS_CACHE .. ".tmp"
  local fw = io.open(tmp, "w")
  if fw then
   fw:write(raw)
   fw:close()
   os.rename(tmp, ALLOWED_IPS_CACHE)
  end
 end
 local ips = {}
 for ip in raw:gmatch("inet6?%s+([%w%.:]+)/") do
  ips[ip:lower()] = true
 end
 return ips
end
local function get_allowed_hosts(hostname)
 local hosts = {}
 ...
 hosts["::1"] = true
 for ip in pairs(get_own_ips()) do
  hosts[ip] = true
 end
 return hosts
end
...
local function is_trusted_host(host_header)
 ...
 local allowed = get_allowed_hosts(hostname)
 ...
 return is_private_ip_literal(host)
end
function action_api()
 local dispatcher = require "luci.dispatcher"
